...
export const StringSelector: React.FC<StringSelectorProps> = ({ selected, onSelect, activeStrings, detectedString }) => {
  return (
    <div className="flex justify-center gap-2 w-full max-w-md mx-auto px-4 mb-3">
      {activeStrings.map((str) => {
        const isSelected = selected === str.name;
        const isDetected = detectedString === str.name;

        return (
          <div key={str.name} className="flex flex-col items-center gap-0.5">
            <button
              onClick={() => onSelect(str.name)}
              className={`
                w-12 h-12 xs:w-14 xs:h-14 flex items-center justify-center rounded-full transition-all duration-300 border-2
                ${isSelected
                  ? 'bg-cyan-500/10 border-cyan-400 text-white shadow-[0_0_15px_rgba(34,211,238,0.5)] scale-105'
                  : isDetected
              ...
              `}
            >
              <span className={`text-base xs:text-lg font-black uppercase ${isSelected || isDetected ? 'drop-shadow-[0_0_5px_rgba(255,255,255,0.5)]' : ''}`}>
                {str.label}
              </span>
            </button>

            {/* Pointer triangle for auto-detected string */}
            <div className={`w-0 h-0 border-l-[5px] border-r-[5px] border-b-[7px] border-l-transparent border-r-transparent transition-all duration-300
              ${isDetected
